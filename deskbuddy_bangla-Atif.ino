// ==================================================
// EDISON SCIENCE CORNER  - ESCLABS (Hybrid Bangla-English for 1.3" SH1106)
// ==================================================
...
#include <Adafruit_SH110X.h> // SH1106 লাইব্রেরি
#include <U8g2_for_Adafruit_GFX.h>
#include "time.h"
#include <math.h>

// আপনার তৈরি করা bitmaps.h ফাইলটি ইমপোর্ট করা হলো
#include "bitmaps.h" 
...
// ==================================================
// 2b. CONFIG PORTAL & SERIAL PROMPT
// ==================================================
...
<label>WiFi SSID</label><input name="ssid" placeholder="WiFi Name" value=")rawliteral";
  html += sSsid;
  html += R"rawliteral(">
...
<label>API Key</label><input name="apikey" placeholder="API key" value=")rawliteral";
  html += sApik;
  html += R"rawliteral(">
<label>City</label><input name="city" placeholder="e.g. Dhaka" value=")rawliteral";
  html += sCity;
  html += R"rawliteral(">
<label>Country Code</label><input name="country" placeholder="e.g. BD" value=")rawliteral";
  html += sCtry;
  html += R"rawliteral(">
</div>
<div class="section"><div class="section-title">Time</div>
<label>Timezone (BD is <+06>-6)</label><input name="tz" placeholder="<+06>-6" value=")rawliteral";
  html += sTz;
  html += R"rawliteral(">
</div>
...
void handleConfigSave() {
  ...
  configServer.send(200, "text/html",
    ...
    "<h2 style='color:#5ba3f5'>Saved!</h2><p>Rebooting...</p></body></html>");
  delay(2000);
  ESP.restart();
}
...
// ==================================================
// নতুন ফাংশন: সিরিয়াল মনিটরে ওয়াইফাই পাসওয়ার্ড প্রম্পট
// ==================================================
void askCredentialsViaSerial() {
  display.clearDisplay();
  display.setFont(NULL);
  display.setCursor(0, 20);
  display.print("WiFi Failed!");
  display.setCursor(0, 40);
  display.print("Check Serial Mon.");
  display.display();
  ...
  Serial.println("======================================");
  Serial.println("Please enter your WiFi Name (SSID) and press Enter:");
  ...
  Serial.println("\nNow, enter your WiFi Password and press Enter:");
  ...
  saveConfig(newSsid, newPass, apiKey, city, countryCode, tzString);

  Serial.println("\nSaved successfully! Rebooting ESP32...");
...
void drawEyelidMask(float x, float y, float w, float h, int mood, bool isLeft) {
  ...
  if (mood == MOOD_SLEEPY) display.fillRect(ix, iy, iw, ih / 2 + 2, SH110X_BLACK);
...
void drawUltraProEye(Eye& e, bool isLeft) {
  ...
  display.fillRoundRect(px, py, iw/2.2, ih/2.2, r / 2, SH110X_BLACK);
  if (iw > 15 && ih > 15) display.fillCircle(px + iw/2.2 - 4, py + 4, 2, SH110X_WHITE);
...
void drawForecastPage() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    display.drawBitmap(centerX - 8, 30, getMiniIcon(fcast[i].iconType), 16, 16, SH110X_WHITE);
    
    String tempStr = getBanglaNumber(fcast[i].temp) + "°";
    int w = u8g2Fonts.getUTF8Width(tempStr.c_str());
...
void drawClock() {
  ...
  display.getTextBounds(ampm, 0, 0, &x1, &y1, &w, &h);
  ...
  String dateNum = getBanglaNumber(t.tm_mday);
  int dateWidth = u8g2Fonts.getUTF8Width(dateNum.c_str());

  int currentDayWidth = bangla_days[dayIndex].width;  
  int currentMonthWidth = bangla_months[monthIndex].width;
  
  int totalWidth = currentDayWidth + 5 + dateWidth + 5 + currentMonthWidth;
  int startX = (128 - totalWidth) / 2;
  int yPos = 64 - imgHeight; 
  
  display.drawBitmap(startX, yPos, bangla_days[dayIndex].array_data, currentDayWidth, imgHeight, SH110X_WHITE);
  startX += currentDayWidth + 5; 
  
  u8g2Fonts.setCursor(startX, yPos + (imgHeight / 1.2)); 
  u8g2Fonts.print(dateNum);
  startX += dateWidth + 5;
  
  display.drawBitmap(startX, yPos, bangla_months[monthIndex].array_data, currentMonthWidth, imgHeight, SH110X_WHITE);
}

void drawWeatherCard() {
  if (WiFi.status() != WL_CONNECTED) {
    display.setFont(NULL); display.setCursor(40, 35); display.print("No WiFi"); return;
  }
...
void playBootAnimation() {
  int cx = 64, cy = 32;
  for (int r = 0; r < 80; r += 4) { display.clearDisplay(); display.fillCircle(cx, cy, r, SH110X_WHITE); display.display(); delay(10); }
  for (int r = 0; r < 80; r += 4) { display.clearDisplay(); display.fillCircle(cx, cy, 80, SH110X_WHITE); display.fillCircle(cx, cy, r, SH110X_BLACK); display.display(); delay(10); }
